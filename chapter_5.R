...
t統計量 <- t分子 / t分母
t統計量
qt(0.025, 19)  # 自由度n-1=20-1=19のt分布で下側確率0.025となるtの値を求める（確率からtの値）
qt(0.975, 19)
qt(0.025, 19, lower.tail=FALSE)
pt(-2.616648, 19)  # 自由度n-1=20-1=19のt分布で下側確率Prob(t<=-2.616648)（tの値から確率）
pt(2.616648, 19, lower.tail=FALSE)
2*pt(2.616648, 19, lower.tail=FALSE)  # 両側検定なので2倍する．（0.05/2でも良い）
t.test(心理学テスト, mu=12)  # 1行でt検定が行われる
